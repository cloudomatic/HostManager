...
//
// An implementation of MuiTable as detailed here: https://mui.com/material-ui/react-table/
//
// 'rows' must have 'id' (a key or unique id) and 'name' as the first two columns.
//
...
function EnhancedTableHead(props) {
  ...
  return (
    <TableHead style={{backgroundColor: props.headerShading}}>
      <TableRow>
        ...
        {props.headCells.map((headCell, index) => (
          <TableCell
            key={headCell.id}
            __align={headCell.numeric ? 'right' : 'left'}
...
export default function MuiTable({rows, headCells, headerShading, itemSelectCallback}) {
  const [order, setOrder] = React.useState('asc');
  const [orderBy, setOrderBy] = React.useState('calories');  // This is column 1, i.e. "name"
  ...
  function descendingComparator(a, b, orderBy) {
    var index = getOrderByColumnIndex(orderBy)
    if (b[index] < a[index] || b[index] === undefined) {
      return -1;
    }
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 0 }} variant="outlined">
